...
module Inferno
  module Generator
    class CernerUSCoreGenerator < Generator::Base
      ...
      def update_cerner_module
        metadata = {}

        metadata[:sequences] = []

        file_path = "./lib/app/cerner_data/goal.yml"
        p "==========#{file_path} before sleep"
        sleep(2)

        p "=========#{file_path} after sleep @@@@@@@@@@@@@@@@@"
        binding.pry
        sequence = YAML.load(File.read(file_path)).with_indifferent_access

        # sequence = File.read(file_path).to_json
        utility = Utility.new
        utility.parse_capability(file_path)
        utility.build_combinations
        utility.process_occurrences
        # utility.date_range
        # utility.acceptheader_formats

        sequence[:happy_path] = utility.generate_testcases[:happy_path]
        sequence[:unhappy_path] = utility.generate_testcases[:unhappy_path]
        metadata[:sequences].push sequence

        # TODO
        # Dir.glob(File.join(__dir__, 'lib/app/cerner_data', '*')).each do |path|
        #   binding.pry
        #   sequence = JSON.parse(File.read(path))
        #   binding.pry
        #   utility = Utility.new
        #   utility.parse_capability(path)
        #   utility.build_combinations
        #   utility.process_occurrences
        #   utility.date_range
        #   utility.acceptheader_formats
        #
        #   sequence[:cerner_capabilities] = utility.generate_testcases
      ...
      def generate_sequence(sequence)
        ...
        template = ERB.new(File.read(File.join(__dir__, 'templates/cerner_sequence.rb.erb')))
        binding.pry
      ...
      def generate_module(module_info)
        ...
        template = ERB.new(File.read(File.join(__dir__, 'templates/cerner_uscore_module.yml.erb')))
